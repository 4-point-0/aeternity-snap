...
export function getSchema(tag: Tag, version?: number): Array<[string, Field]> {
  const schemas = txSchema.filter((s: any) => s.tag.constValue === tag);
  if (schemas.length === 0)
    throw new SchemaNotFoundError(`${Tag[tag]} (${tag})`, 0);
  version ??= Math.max(
    ...schemas.map((schema: any) => schema.version.constValue)
  );
  const schema = schemas.find((s: any) => s.version.constValue === version);
  if (schema == null)
  ...
  const binary = schema.map(([key, field]) =>
    field.serialize(
      params[key],
      {
        ...
        unpackTx,
        buildTx,
        rebuildTx: (overrideParams: any) =>
          buildTx({ ...params, ...overrideParams }),
      },
      params
    )
  );
...
export function unpackTx<TxType extends Tag>(
  ...
  if (txType != null && txType !== tag)
    throw new DecodeError(
      `Expected transaction to have ${Tag[txType]} tag, got ${Tag[tag]} instead`
    );
  if (binary.length !== schema.length) {
    throw new ArgumentError(
      "Transaction RLP length",
      schema.length,
      binary.length
    );
  }
  return schema.reduce<any>((acc, [name, field], index) => {
    // eslint-disable-next-line @typescript-eslint/no-use-before-define
    const deserialized = field.deserialize(binary[index] as BinaryData, {
      unpackTx,
    });
 ...
 */
export function buildTxHash(
  rawTx: Encoded.Transaction | Uint8Array
): Encoded.TxHash {
  const data =
    typeof rawTx === "string" && rawTx.startsWith("tx_")
      ? decode(rawTx)
      : rawTx;
...
export function buildContractIdByContractTx(
  ...
  const params: any = unpackTx(contractTx);
