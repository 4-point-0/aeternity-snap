...
type PreserveOptional<NewType, OrigType> = OrigType extends undefined
  ? NewType | undefined
  : NewType;
export type TransformNodeType<Type> = Type extends (
  ...args: infer Args
) => infer Ret
  ...
  : Type;
type NodeTransformedApi = new (
  ...args: ConstructorParameters<typeof NodeApi>
) => {
  [Name in keyof InstanceType<typeof NodeApi>]: Name extends
    | "pipeline"
    | "sendRequest"
  ...
  constructor(
    ...
    super(url, {
      ...
      additionalPolicies: [
        ...
        genErrorFormatterPolicy((body: any) => ` ${body.reason}`),
      ],
      ...options,
    });
    if (!ignoreVersion) {
