...
export function getExecutionCost(
  ...
  const params: any = unpackTx(transaction);
...
export function getExecutionCostBySignedTx(
  ...
  const params: any = unpackTx(transaction, Tag.SignedTx);
  ...
  if (!isInnerTx && !isNotInnerTx)
    throw new TransactionError("Can't verify signature");
...
export async function getExecutionCostUsingNode(
  ...
  node: Node,
  {
    isMined,
    ...options
  }: { isMined?: boolean } & Parameters<typeof getExecutionCost>[1] = {}
): Promise<bigint> {
  let params: any = unpackTx(transaction);
  ...
  if (
    ...
    if (combinedInfo == null) {
      throw new InternalError(
        `callInfo and gaInfo is not available for transaction ${txHash}`
      );
    }
  ...
  if (options.queryFee == null && Tag.OracleResponseTx === params.tag) {
    options.queryFee = (
      await node.getOracleByPubkey(params.oracleId)
    ).queryFee.toString();
  }

  if (
    options.isInitiator == null &&
    Tag.ChannelSettleTx === params.tag &&
    isMined !== true
  ) {
