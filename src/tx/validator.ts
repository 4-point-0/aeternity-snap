...
type Validator = (
  ...
  options: {
    ...
    account: any & { id: Encoded.AccountAddress };
...
async function verifyTransactionInternal(
  ...
  const [account, { height }, { consensusProtocolVersion, nodeNetworkId }] =
    await Promise.all([
      node
        .getAccountByPubkey(address)
        .catch((error: any) => {
          if (!isAccountNotFoundError(error)) throw error;
          return { id: address, balance: BigInt(0), nonce: 0 };
        })
        ...
        .then(
          (acc: {
            ...
            balance: bigint;
            nonce: number;
          }) => ({ ...acc, id: acc.id as Encoded.AccountAddress })
        ),
...
validators.push(
  (tx: any, { account, nodeNetworkId, parentTxTypes }) => {
  ...
  async (tx, { account, parentTxTypes, node }) => {
    ...
    const cost = await getExecutionCostUsingNode(buildTx(tx), node).catch(() =>
      BigInt(0)
    );
  ...
  (tx: any, { account }) => {
    ...
    if (
      tx.tag === Tag.SignedTx &&
      account.kind === "generalized" &&
      tx.signatures.length !== 0
    ) {
      message =
        "Generalized account can't be used to generate SignedTx with signatures";
    }
  ...
  (tx: any, { consensusProtocolVersion }) => {
    const oracleCall = Tag.Oracle === tx.tag || Tag.OracleRegisterTx === tx.tag;
    const contractCreate =
      Tag.ContractCreateTx === tx.tag || Tag.GaAttachTx === tx.tag;
    const contractCall =
      Tag.ContractCallTx === tx.tag || Tag.GaMetaTx === tx.tag;
    const type =
    ...
    if (
      ...
      return [
        {
          message: `ABI/VM version ${JSON.stringify(
            ctVersion
          )} is wrong, supported is: ${JSON.stringify(protocol)}`,
  ...
  async (tx: any, { node }) => {
    ...
    try {
      ...
      if (!(error instanceof RestError) || error.response?.bodyAsText == null)
        throw error;
      return [
        {
