...
export default function genNonceField<SenderKey extends string>(
  ...
  prepare: (
    ...
    options: { [key in SenderKey]: string } & { strategy?: any; onNode?: Node }
  ) => Promise<number>;
  ...
  return {
    ...
    async prepare(value, params, options) {
      ...
      if (onNode == null)
        throw new ArgumentError("onNode", requirement, onNode);
      if (senderId == null)
        throw new ArgumentError("senderId", requirement, senderId);
      return (
        await onNode
